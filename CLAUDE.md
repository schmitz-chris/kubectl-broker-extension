...
6. **Import hygiene**: Keep imports grouped as `stdlib`, third-party, then `kubectl-broker/...` packages with a blank line between groups. If `goimports` isn’t available locally, reorder manually and run `gofmt` before committing. Avoid leaving project imports mixed in with standard/third-party packages.
